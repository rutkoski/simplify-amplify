...
class UsersController extends Amplify_FormController
{
  ...
  protected function initialize()
  {
    ...
    $password = new Simplify_Form_Element_Password('user_password', 'Password');

    $email = new Simplify_Form_Element_Email('user_email', 'Email');
  ...
  public function onRender(Simplify_Form_Action $action)
  {
    ...
    foreach($data as &$row) {
      if ($row[Simplify_Form::ID] == 1) {
        $row['elements']['groups']['controls'] = 'This user belongs to every group';
        $row['elements']['permissions']['controls'] = 'This user has all permissions';
      }
    }
