...
class Account
{
  ...
  public static function requestPasswordChange($email)
  {
    ...
    if (empty($user)) {
      throw new Simplify_ValidationException('User not found');
    }
  ...
  public static function changePassword($email, $auth, $pass_a, $pass_b)
  {
    ...
    if (empty($user)) {
      throw new Simplify_ValidationException('User not found or wrong authorization code.');
    }
    ...
    if ($pass_a == $empty || $pass_a != $pass_b) {
      throw new Simplify_ValidationException('Invalid password or passwords do not match.');
    }
  ...
  public static function addUserGroup($user_id, $group_id)
  {
    if (is_string($group_id)) {
      $group_id = s::db()->query()->from(ACL_GROUPS_TABLE)->select('group_id')->where('group_name = ?')->execute($group_id)->fetchOne();
  ...
  public static function addUserPermission($user_id, $permission_id)
  {
    if (is_string($permission_id)) {
      $permission_id = s::db()->query()->from(ACL_PERMISSIONS_TABLE)->select('permission_id')->where('permission_name = ?')->execute($permission_id)->fetchOne();
