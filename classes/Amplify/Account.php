...
class Account
{
  ...
  protected static function loadUserAcl($user)
  {
    ...
    self::$acl = \Simplify::db()->query($sql)->execute($user['user_id'], $user['user_id'])->fetchCol();
  }
  ...
  public static function login($email, $password)
  {
    ...
    \Simplify::db()->update(\Simplify::config()->get('amp:tables:users'), array('access_token' => 1), 'user_id = :user_id')->execute($data);
  ...
  public static function createUser($email, $password)
  {
    ...
    $user_id = \Simplify::db()->query()->from(\Simplify::config()->get('amp:tables:users'))->select('user_id')->where('user_email = ?')->execute($email)->fetchOne();

    if (empty($user_id)) {
      \Simplify::db()->insert(\Simplify::config()->get('amp:tables:users'), $data)->execute($data);
  ...
  public static function createGroup($name, $throw = true)
  {
    ...
    $group_id = \Simplify::db()->query()->from(\Simplify::config()->get('amp:tables:groups'))->select('group_id')->where('group_name = :group_name')->execute($data)->fetchOne();

    if (empty($group_id)) {
      \Simplify::db()->insert(\Simplify::config()->get('amp:tables:groups'), $data)->execute($data);
  ...
  public static function createPermission($name, $description = '')
  {
    ...
    $permission_id = \Simplify::db()->query()->from(\Simplify::config()->get('amp:tables:permissions'))->select('permission_id')->where('permission_name = ?')->execute($name)->fetchOne();

    if (empty($permission_id)) {
      \Simplify::db()->insert(\Simplify::config()->get('amp:tables:permissions'), $data)->execute($data);
  ...
  public static function addUserGroup($user_id, $group_id)
  {
    if (is_string($group_id)) {
      $group_id = \Simplify::db()->query()->from(\Simplify::config()->get('amp:tables:groups'))->select('group_id')->where('group_name = ?')->execute($group_id)->fetchOne();
    ...
    \Simplify::db()->query('INSERT IGNORE INTO '.\Simplify::config()->get('amp:tables:groups_users').' VALUES (:group_id, :user_id)')->execute($data);
  }
  ...
  public static function addUserPermission($user_id, $permission_id)
  {
    ...
    \Simplify::db()->query('INSERT IGNORE INTO '.\Simplify::config()->get('amp:tables:permissions_users').' VALUES (:permission_id, :user_id)')->execute($data);
  }
